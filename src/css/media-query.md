...
```mermaid
flowchart LR
    Root((大屏自适应)) --> Why[为什么需要]
    Root --> Solutions[市面常见实现方案]
    Root --> Cases[最佳实践案例]
    ...
    Solutions --> Sol1[媒体查询]
    Solutions --> Sol2[Scale缩放]
    Solutions --> Sol3[百分比布局]
    Solutions --> Sol4[监听尺寸变化]
    Solutions --> Sol5[浏览器缩放监听]
...
```mermaid
graph LR
    classDef solution fill:#f9f7f7,stroke:#333,stroke-width:1px
    classDef advantage fill:#d3f8e2,stroke:#333,stroke-width:1px
    classDef disadvantage fill:#f8d3d3,stroke:#333,stroke-width:1px

    subgraph "媒体查询方案"
        direction TB
        MQ[媒体查询]:::solution --> MQ_A1[优点: 原生CSS支持]:::advantage
        MQ --> MQ_A2[优点: 精确控制不同尺寸下的样式]:::advantage
        MQ --> MQ_D1[缺点: 需要维护多套样式]:::disadvantage
        MQ --> MQ_D2[缺点: 断点设置复杂]:::disadvantage
    end

    subgraph "Scale缩放方案"
        direction TB
        SC[Scale缩放] --> SC_A1[优点: 保持设计稿比例]:::advantage
        SC --> SC_A2[优点: 实现简单]:::advantage
        SC --> SC_D1[缺点: 可能出现模糊]:::disadvantage
    end

    subgraph "百分比布局方案"
        direction TB
        PL[百分比布局] --> PL_A1[优点: 流式布局]:::advantage
        PL --> PL_A2[优点: 适应性强]:::advantage
        PL --> PL_D1[缺点: 复杂布局难实现]:::disadvantage
    end
...
<template>
  <div class="dashboard-wrapper" ref="wrapper">
    <!-- 缩放容器 -->
    <div class="dashboard-container" ref="container" :style="containerStyle">
      <!-- 大屏内容插槽 -->
      <slot></slot>
  ...
  export default {
    ...
    methods: {
      ...
      adjustIframeSize() {
        ...
        console.log(
          `调整iframe尺寸: ${this.frameWidth}x${this.frameHeight}, 缩放比例: ${zoomFactor}`
        );
      },
      // 监听浏览器缩放比例变化
      listenForZoomChanges() {
...
<template>
  <div class="app-container">
    ...
    <!-- 方案2：使用iframe嵌入式大屏 -->
    <EnhancedBiReport :width="1200" :height="800" code="dashboard123" />
